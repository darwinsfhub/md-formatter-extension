...
class MarkdownParser {
  ...
  parseInline(text) {
    ...
    // Include explicit link styling for better compatibility with Google Docs and Word
    text = text.replace(/\[\^(\w+)\]/g, (_, id) => {
      const supStyle = this.options.includeStyles ? ' style="vertical-align:super;font-size:0.8em;"' : '';
      const linkStyle = this.options.includeStyles ? ' style="color:#1a73e8;text-decoration:none;"' : '';
      return `<sup${supStyle}><a href="#fn-${id}" id="fnref-${id}"${linkStyle}>[${id}]</a></sup>`;
    });
