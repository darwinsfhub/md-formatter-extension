...
class DiagramHandler {
  ...
  /**
   * Process entire document - detects ASCII diagrams anywhere, not just in code blocks
   * This handles mixed content documents with text and diagrams
   *
   * @param {string} markdown - The full markdown document
   * @param {object} options - Processing options
   * @returns {string} - Processed markdown with diagrams converted to images
   */
  processFullDocument(markdown, options = {}) {
    const {
      convertAsciiToSvg = true,
      renderMermaid = true,
      autoDetectDiagrams = true,
      minDiagramLines = 3
    } = options;

    let processed = markdown;

    // Step 1: Process explicit code blocks (fenced with ```)
    processed = this.processCodeBlocks(processed, { convertAsciiToSvg, renderMermaid });

    // Step 2: Process indented code blocks (4 spaces)
    processed = this.processIndentedCodeBlocks(processed, { convertAsciiToSvg });

    // Step 3: Auto-detect ASCII diagrams in regular text (if enabled)
    if (autoDetectDiagrams) {
      processed = this.autoDetectAndConvertDiagrams(processed, { minDiagramLines });
    }
   ...
   * Process fenced code blocks (``` ... ```)
   */
  processCodeBlocks(markdown, options = {}) {
    const { convertAsciiToSvg = true, renderMermaid = true } = options;

    let processed = markdown;

    // Process ASCII/diagram code blocks
    if (convertAsciiToSvg) {
      processed = processed.replace(
        /```(ascii|diagram|art|box|text)?\n([\s\S]*?)```/g,
        (match, lang, content) => {
          // Skip if it's a known programming language
          if (lang && ['js', 'javascript', 'python', 'java', 'cpp', 'c', 'html', 'css', 'json', 'xml', 'sql', 'bash', 'sh', 'yaml', 'yml', 'typescript', 'ts', 'go', 'rust', 'ruby', 'php'].includes(lang.toLowerCase())) {
            return match;
          }

          // Check if content looks like ASCII diagram
          if (this.isAsciiDiagram(content) || ['ascii', 'diagram', 'art', 'box'].includes(lang)) {
            const imgTag = this.asciiToImgTag(content.trimEnd());
            return imgTag;
          }
          return match;
   ...
   */
  processIndentedCodeBlocks(markdown, options = {}) {
    const { convertAsciiToSvg = true } = options;

    if (!convertAsciiToSvg) return markdown;
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (isIndented && !inCodeBlock) {
        ...
        codeBlockStartIndex = result.length;
      } else if (inCodeBlock && (isIndented || isEmpty)) {
        // Continue code block
        codeBlockLines.push(isIndented ? line.replace(/^(    |\t)/, '') : '');
      } else if (inCodeBlock) {
        // End of code block
        const codeContent = codeBlockLines.join('\n').trimEnd();

        if (this.isAsciiDiagram(codeContent)) {
          // Convert to image
          result.push(this.asciiToImgTag(codeContent));
        } else {
          // Keep as indented code
          codeBlockLines.forEach(l => result.push('    ' + l));
        }
    ...
    // Handle code block at end of document
  ...
  /**
   * Auto-detect ASCII diagrams in regular text (not in code blocks)
   * Looks for contiguous blocks of lines that appear to be diagrams
   */
  autoDetectAndConvertDiagrams(markdown, options = {}) {
    const { minDiagramLines = 3 } = options;

    const lines = markdown.split('\n');
    const result = [];
    let potentialDiagram = [];
    let inFencedBlock = false;

    for (let i = 0; i < lines.length; i++) {
      const line = lines[i];

      // Track fenced code blocks to avoid processing them twice
      if (line.startsWith('```')) {
        inFencedBlock = !inFencedBlock;

        // Flush any pending diagram
        if (potentialDiagram.length > 0) {
          this.flushPotentialDiagram(potentialDiagram, result, minDiagramLines);
          potentialDiagram = [];
        }
      ...
      // Check if this line looks like part of a diagram
      if (this.lineAppearsToBePartOfDiagram(line)) {
        potentialDiagram.push(line);
      } else {
        // Flush any pending diagram
        if (potentialDiagram.length > 0) {
          this.flushPotentialDiagram(potentialDiagram, result, minDiagramLines);
  ...
  /**
   * Check if a single line appears to be part of an ASCII diagram
   */
  lineAppearsToBePartOfDiagram(line) {
    if (!line || line.trim().length === 0) return false;

    // Count diagram-like characters
    const diagramChars = /[│─┌┐└┘├┤┬┴┼═║╔╗╚╝╠╣╦╩╬+\-|<>^v\/\\*]/g;
    const matches = line.match(diagramChars);
    const diagramCharCount = matches ? matches.length : 0;

    // Check character density
    const nonSpaceChars = line.replace(/\s/g, '').length;
    if (nonSpaceChars === 0) return false;

    const diagramCharRatio = diagramCharCount / nonSpaceChars;

    // Line is likely part of diagram if:
    // - High ratio of diagram characters (>30%)
    // - Or contains box drawing characters
    // - Or has specific patterns
    if (diagramCharRatio > 0.3) return true;
    if (/[│─┌┐└┘├┤┬┴┼═║╔╗╚╝╠╣╦╩╬]/.test(line)) return true;
    if (/^\s*[+|]\s*[-=]+\s*[+|]\s*$/.test(line)) return true; // +---+ or |---|
    if (/^\s*[+|].*[+|]\s*$/.test(line) && line.includes('  ')) return true; // | text | with spacing
    if (/^\s*[-=]{3,}\s*$/.test(line)) return true; // --- or ===

    return false;
  }

  /**
   * Flush accumulated potential diagram lines
   */
  flushPotentialDiagram(lines, result, minLines) {
    if (lines.length >= minLines) {
      const diagramText = lines.join('\n');
      if (this.isAsciiDiagram(diagramText)) {
        result.push(this.asciiToImgTag(diagramText));
        return;
      }
    }

    // Not a diagram, add lines as-is
