...
class ClipboardManager {
  ...
  /**
   * Wrap HTML content in a proper document structure for better compatibility
   * This ensures links, formatting, and styles are preserved when pasting
   * into Google Docs, Microsoft Word, and other rich text applications.
   *
   * Key features:
   * - StartFragment/EndFragment markers for Windows clipboard compatibility
   * - Meta charset for proper character encoding
   * - Inline styles for links to ensure they remain clickable
   * - Support for superscript footnote links
   */
  wrapHtmlForClipboard(htmlContent) {
    return `<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
...
<style>
  ...
  a[href] { color: #1a73e8; text-decoration: underline; }
  sup { vertical-align: super; font-size: 0.8em; }
  sup a { color: #1a73e8; text-decoration: none; }
...
<!--EndFragment-->
  ...
  /**
   * Ensure all links have proper attributes for maximum compatibility
   */
  enhanceLinksForClipboard(html) {
    // Add target and rel attributes to links if not present
    // This improves compatibility with some applications
    return html.replace(
      /<a\s+href="([^"]+)"([^>]*)>/gi,
      (match, url, rest) => {
        // Don't modify internal anchor links
        if (url.startsWith('#')) {
          return match;
        }
        // Ensure style attribute exists for link color
        if (!rest.includes('style=')) {
          rest += ' style="color:#1a73e8;text-decoration:underline;"';
        }
  ...
  async copyFormatted(htmlContent, plainTextContent) {
    try {
      // Enhance links and wrap HTML in proper document structure
      const enhancedHtml = this.enhanceLinksForClipboard(htmlContent);
      const wrappedHtml = this.wrapHtmlForClipboard(enhancedHtml);
      ...
      return this.fallbackCopy(wrappedHtml, plainTextContent);

    } catch (error) {
      console.error('Clipboard write failed:', error);

      // Try fallback with wrapped HTML
      try {
        const enhancedHtml = this.enhanceLinksForClipboard(htmlContent);
        return this.fallbackCopy(this.wrapHtmlForClipboard(enhancedHtml), plainTextContent);
      } catch (fallbackError) {
        return {
          success: false,
